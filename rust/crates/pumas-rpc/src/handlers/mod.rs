...
async fn dispatch_admitted_command(
    ...
    let result: pumas_library::Result<RpcOutcome> = match command {
        ...
        #[cfg(feature = "inference-plugins")]
        RpcCommand::CheckVersionDependencies { app_id, tag } => {
            versions::check_version_dependencies(state, &app_id, &tag)
                .await
                .map(RpcOutcome::CheckVersionDependencies)
        }
        #[cfg(feature = "inference-plugins")]
        RpcCommand::GetReleaseDependencies { app_id, tag } => {
            versions::get_release_dependencies(state, &app_id, &tag)
                .await
        ...
        #[cfg(feature = "inference-plugins")]
        RpcCommand::Legacy { method, params } if method == "cancel_installation" => {
            versions::cancel_installation(state, &params)
                .await
        ...
        #[cfg(feature = "inference-plugins")]
        RpcCommand::IsOllamaRunning => process::is_ollama_running(state)
            .await
            .map(RpcOutcome::RuntimeRunning),
        #[cfg(feature = "inference-plugins")]
        RpcCommand::IsTorchRunning => process::is_torch_running(state)
            .await
            .map(RpcOutcome::RuntimeRunning),
        #[cfg(feature = "inference-plugins")]
...
mod tests {
    ...
    #[tokio::test]
    async fn switch_version_rpc_preserves_no_manager_and_feature_gate() {
        let temp = TempDir::new().unwrap();
        let state = Arc::new(test_support::build_test_app_state(temp.path()).await);
        let request = Bytes::from(
            serde_json::to_vec(&json!({
                "jsonrpc":"2.0","id":"switch-fixture","method":"switch_version",
        ...
        assert_eq!(
            wire,
            json!({"jsonrpc":"2.0","id":"switch-fixture","error":{
    ...
    #[tokio::test]
    async fn check_version_dependencies_rpc_admission_and_errors() {
        let temp = TempDir::new().unwrap();
        let state = Arc::new(test_support::build_test_app_state(temp.path()).await);
        for (params, accepted) in crate::contract::install_version_requests() {
            let request = Bytes::from(serde_json::to_vec(&json!({
                "jsonrpc":"2.0", "id":"deps-fixture", "method":"check_version_dependencies", "params":params,
            })).unwrap());
            let response = handle_rpc(State(state.clone()), request)
                .await
                .into_response();
    ...
    #[tokio::test]
    async fn get_release_dependencies_rpc_admission_and_errors() {
        let temp = TempDir::new().unwrap();
        let state = Arc::new(test_support::build_test_app_state(temp.path()).await);
        for (params, accepted) in crate::contract::install_version_requests() {
            let request = Bytes::from(serde_json::to_vec(&json!({
    ...
    #[tokio::test]
    async fn runtime_running_rpc_admission_without_process_manager() {
        let temp = TempDir::new().unwrap();
        let state = Arc::new(test_support::build_test_app_state(temp.path()).await);
        for method in ["is_ollama_running", "is_torch_running"] {
            for (params, accepted) in crate::contract::runtime_launch_requests() {
                let mut request = json!({"jsonrpc":"2.0","id":"running-fixture","method":method});
                if let Some(params) = params {
                    request["params"] = params;
                }
                let response = handle_rpc(
                ...
                if accepted {
                    assert_eq!(
                        wire,
                        json!({"jsonrpc":"2.0","id":"running-fixture","result":false})
                    );
                    continue;
                }
                #[cfg(feature = "inference-plugins")]
                ...
                assert_eq!(
                    wire,
                    json!({"jsonrpc":"2.0","id":"running-fixture","error":{"code":error.code,"message":error.message,"data":{"class":error.class.as_str()}}})
                );
            }
        }
    }
    ...
    #[tokio::test]
    async fn runtime_running_core_without_process_manager_returns_false() {
        let temp = TempDir::new().unwrap();
        // This helper explicitly disables process management; never use a real runtime manager.
        let api = test_support::build_test_api(temp.path()).await;
        for raw in [api.is_ollama_running().await, api.is_torch_running().await] {
            assert!(!raw);
            assert_eq!(
                serde_json::to_value(crate::contract::RuntimeRunningOutcome::new(raw)).unwrap(),
                json!(false)
            );
        }
    }
    ...
    #[tokio::test]
    async fn cancel_installation_rpc_preserves_no_manager_and_feature_gate() {
        let temp = TempDir::new().unwrap();
        let state = Arc::new(test_support::build_test_app_state(temp.path()).await);
        let request = Bytes::from(
            serde_json::to_vec(&json!({
                "jsonrpc":"2.0","id":"cancel-fixture","method":"cancel_installation",
                "params":{"app_id":"unregistered-runtime"},
            }))
            .unwrap(),
        );
        ...
        assert_eq!(
            wire,
            json!({"jsonrpc":"2.0","id":"cancel-fixture","result":{"success":false}})
        );
        #[cfg(not(feature = "inference-plugins"))]
        assert_eq!(
            wire,
            json!({"jsonrpc":"2.0","id":"cancel-fixture","error":{
                "code":-32601,"message":"The requested method is not supported.","data":{"class":"not_found"},
            }})
        );
    }
