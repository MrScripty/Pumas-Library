...
def main():
    ...
    if all_violations:
        ...
        write_out(f"{RED}{'━' * 60}{NC}")
        write_out("")
        write_out("Please use CSS theme variables from frontend/src/index.css instead.")
        write_out("")
        write_out("Available theme variables include:")
        write_out(f"  {GREEN}•{NC} --surface-interactive, --surface-low, --surface-overlay")
        write_out(f"  {GREEN}•{NC} --text-primary, --text-secondary, --text-tertiary")
        write_out(f"  {GREEN}•{NC} --accent-success, --accent-error, --accent-warning")
        write_out(f"  {GREEN}•{NC} --border-default, --border-control")
        write_out("")
        write_out("Usage:")
        write_out(f'  {GREEN}✅{NC} className="bg-[hsl(var(--surface-interactive))]"')
        write_out(f'  {GREEN}✅{NC} className="text-[hsl(var(--text-tertiary))]"')
        write_out("")
        write_out("See frontend/src/index.css for the complete list of theme variables.")
