  ...
  return [
    {
      ...
      check: ({ platformService }) =>
        commandExistsFn(platformService.cargoCommand, ['--version']),
      install: async () => {
    ...
    },
    {
      name: 'corepack',
      check: ({ platformService }) =>
        commandExistsFn(platformService.corepackCommand, ['--version']),
      install: async () => {
    ...
    },
    {
      ...
      check: ({ context }) =>
        existsSyncFn(path.join(context.repoRoot, 'node_modules')) &&
        existsSyncFn(path.join(context.frontendDir, 'node_modules')) &&
        existsSyncFn(path.join(context.electronDir, 'node_modules')),
      install: async ({ context, platformService }) => {
        await runCommandFn(platformService.corepackCommand, corepackPnpmArgs(installArgs()), {
