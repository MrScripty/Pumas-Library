...
- 2026-04-22: App shell rendering, app-panel prop construction, startup checks, and selected-app version routing moved from `App.tsx` into `frontend/src/components/AppShell.tsx`, `frontend/src/components/AppShellPanels.ts`, `frontend/src/hooks/useAppStartupChecks.ts`, and `frontend/src/hooks/useSelectedAppVersions.ts`, with focused tests; `App.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
...
- 2026-04-22: Remote Hugging Face download start orchestration moved from `ModelManager.tsx` into `frontend/src/components/ModelManagerRemoteDownload.ts`, with successful start, backend failure, and auth-required error tests; `ModelManager.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
...
- 2026-04-22: Local model empty-library and no-match rendering moved from `LocalModelsList.tsx` into `frontend/src/components/LocalModelsEmptyState.tsx`, with picker, disabled, default empty, and clear-filter tests; the file-size baseline now ratchets `LocalModelsList.tsx` to 374 effective lines.
- 2026-04-22: Local model format, quant, size, dependency, and partial-error rendering moved from `LocalModelsList.tsx` into `frontend/src/components/LocalModelMetadataSummary.tsx`, with fallback, dependency, and partial-error tests; the file-size baseline now ratchets `LocalModelsList.tsx` to 348 effective lines.
- 2026-04-22: Local model name metadata access and status badges moved from `LocalModelsList.tsx` into `frontend/src/components/LocalModelNameButton.tsx`, with modified-click and badge tests; the file-size baseline now ratchets `LocalModelsList.tsx` to 304 effective lines.
- 2026-04-22: Local model group header rendering moved from `LocalModelsList.tsx` into `frontend/src/components/LocalModelGroupHeader.tsx`, with category/count tests; `LocalModelsList.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Import embedded-metadata disclosure and on-demand metadata loading moved from `useModelImportWorkflow.ts` into `frontend/src/components/model-import/useEmbeddedMetadataToggles.ts`, with loading, cached, unsupported, and all-field toggle tests; the file-size baseline now ratchets `useModelImportWorkflow.ts` to 391 effective lines.
- 2026-04-22: Import sharded-file detection and expansion state moved from `useModelImportWorkflow.ts` into `frontend/src/components/model-import/useShardedSetDetection.ts`, with detection, entry annotation, expansion, and clear-state tests; the file-size baseline now ratchets `useModelImportWorkflow.ts` to 354 effective lines.
- 2026-04-22: Import metadata lookup execution moved from `useModelImportWorkflow.ts` into `frontend/src/components/model-import/modelImportMetadataLookup.ts`, with embedded match, bundle lookup, invalid file, and progress tests; `useModelImportWorkflow.ts` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Import expanded metadata details moved from `ImportLookupCard.tsx` into `frontend/src/components/model-import/ImportMetadataDetails.tsx`, with Hugging Face field, source-switching, linked embedded metadata, and hidden-field disclosure tests; `ImportLookupCard.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Install-dialog background release-size calculation moved from `InstallDialog.tsx` into `frontend/src/hooks/useReleaseSizeCalculation.ts`, with size calculation, skipped-state, session guard, and close/reopen reset tests; the file-size baseline now ratchets `InstallDialog.tsx` to 390 effective lines.
- 2026-04-22: Install-dialog log-path and release-link opening moved from `InstallDialog.tsx` into `frontend/src/hooks/useInstallDialogLinks.ts`, with backend bridge, unavailable bridge, and browser fallback tests; the file-size baseline now ratchets `InstallDialog.tsx` to 356 effective lines.
- 2026-04-22: Install-dialog modal/page frame rendering, backdrop dismissal, Escape handling, and focus restoration moved from `InstallDialog.tsx` into `frontend/src/components/InstallDialogFrame.tsx`, with modal and page-frame tests; `InstallDialog.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Conflict-resolution default decisions, counts, bulk updates, expanded row state, and async apply handling moved from `ConflictResolutionDialog.tsx` into `frontend/src/hooks/useConflictResolutions.ts`, with default, individual, bulk, expansion, and apply tests; the file-size baseline now ratchets `ConflictResolutionDialog.tsx` to 379 effective lines.
- 2026-04-22: Conflict row summary, selector, expanded path details, and per-action descriptions moved from `ConflictResolutionDialog.tsx` into `frontend/src/components/ConflictResolutionItem.tsx`, with summary, selector, and expanded-detail tests; `ConflictResolutionDialog.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Metadata modal frame rendering, header actions, backdrop dismissal, Escape handling, and focus restoration moved from `ModelMetadataModal.tsx` into `frontend/src/components/ModelMetadataModalFrame.tsx`, with modal shell, dismissal, and disabled-refetch tests; `ModelMetadataModal.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Remote model metadata summary rendering moved from `RemoteModelListItem.tsx` into `frontend/src/components/RemoteModelSummary.tsx`, with metadata, developer-search, auth, and retry tests; `RemoteModelListItem.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Torch active-slot rendering moved from `TorchModelSlotsSection.tsx` into `frontend/src/components/app-panels/sections/TorchActiveSlots.tsx`, with slot badge, device memory, unload control, and size-formatting tests; `TorchModelSlotsSection.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
- 2026-04-22: Registered Ollama model rendering moved from `OllamaModelSection.tsx` into `frontend/src/components/app-panels/sections/OllamaRegisteredModels.tsx`, with load, unload, delete, loaded-state, disabled-control, and size-formatting tests; `OllamaModelSection.tsx` is now below the 300 effective-line standard and has been removed from the oversized file baseline.
