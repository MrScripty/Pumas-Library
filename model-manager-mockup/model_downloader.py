...
class ModelDownloader:
    ...
    def _verify_hashes(self, model_dir: Path, manifest: ModelManifest) -> bool:
        ...
        if not target_file:
            ...
            target_file = next(
                (
                    p
                    for p in candidates
                    if p.is_file() and p.suffix in {".gguf", ".safetensors", ".ckpt", ".pt", ".bin"}
                ),
                None,
            )
        ...
        if manifest.hash_sha256:
            ...
            if computed != manifest.hash_sha256.lower():
                logger.error(
                    f"SHA256 mismatch for {target_file}: expected {manifest.hash_sha256}, got {computed}"
                )
                verified = False
            else:
    ...
    def download_model(self, manifest: ModelManifest) -> Path:
        model_type = (
            "llm" if any(t in manifest.tags for t in ["llm", "text-generation"]) else "diffusion"
        )
