...
| ------- | ----- | -------------- |
| App/plugin identity | Plugin metadata, app registry, frontend shell | Describes sidebar identity, install/version metadata when applicable, default URL, icon, and panel entry. This is related to a runtime provider but not the same contract. Torch can remain an app/sidecar without becoming a runtime-profile provider in this plan. |
...
| Launch/runtime strategy | Runtime lifecycle and ONNX provider session modules | Provider behavior selects a typed strategy such as binary process, in-process Rust runtime, or external-only. ONNX uses an in-process Rust ONNX Runtime session manager; Ollama and llama.cpp use binary processes. Torch's existing sidecar is not the ONNX implementation target. |
...
| Serving adapter | Provider-specific serving modules | RPC serving handlers parse/validate boundary input, then call provider adapters for load, unload, status/list, provider-side model id, alias defaulting, idempotency, runtime support checks, and in-process ONNX session access. |
| Gateway endpoint capability | Gateway proxy/helper adapters and provider behavior | `/v1/*` routing checks whether the selected provider supports the requested OpenAI-compatible endpoint before forwarding or executing in-process. ONNX supports `/v1/models` and `/v1/embeddings` in the first slice only. |
...
| ONNX Rust provider control contracts | Rust ONNX provider/session module | Rust validated request/session types plus shared validation helpers | Invalid path/root escape, invalid model id, load/unload idempotency, status shape tests |
| ONNX embedding payloads | Pumas gateway plus Rust ONNX provider adapter | Gateway request parser and Rust validated embedding request types for model, input, dimensions, batch/token limits | Response shape, dimension, batch ordering, invalid dimensions, oversized request tests |
