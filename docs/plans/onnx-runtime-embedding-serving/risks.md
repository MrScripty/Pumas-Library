...
| ONNX package tokenizer/config files may live above the graph file directory. | Medium | Tokenizer discovery now searches from the `.onnx` file directory up to the validated model root, so package-root tokenizer files work without allowing root escapes. Config/model-metadata discovery still needs the same explicit contract before inference defaults rely on it. |
