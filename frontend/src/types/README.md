...
- `api-serving.ts` is the renderer contract for model row/modal serve flows.
  ...
  components may keep only unsaved serving form drafts locally. Serving update
  feeds are invalidation hints; when `snapshot_required` is true, renderer code
  should refresh `get_serving_status`.
