...
## Problem
The model index owns more than search rows: it also persists dependency profile contracts and metadata override history. Those concerns need transactional SQLite access through the same `ModelIndex` connection, but leaving all of that logic in `model_index.rs` makes the index boundary harder to review and maintain.
