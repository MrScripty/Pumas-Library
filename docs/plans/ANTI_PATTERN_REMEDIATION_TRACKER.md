...
| T5 | Harden shell/desktop-entry command generation by escaping script path arguments and script-interpolated quoted values. | Completed | `fix(shortcuts): escape desktop exec args and generated script values` |
...
| T5 | `LAUNCHER-STANDARDS.md` | Add explicit escaping requirements for `.desktop` Exec argument construction and generated shell script interpolation (paths/tags/URLs inside quoted strings). |
