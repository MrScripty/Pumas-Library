...
- Runtime lifecycle code currently has provider-specific launch branches.
  Implementation must add ONNX launch behavior through the same lifecycle owner,
  with stale PID cleanup, health checks, cancellation, and managed state
  isolation.
...
- Model-library metadata currently recognizes a narrow format set.
  ...
  string checks.
- Sidecar implementation is a new process boundary. It must start with a
  documented README contract, runtime validation module, explicit resource
  limits, lifecycle ownership, fake-session tests, and then real ONNX Runtime
  execution.
...
| RPC serving handler | `rust/crates/pumas-rpc/src/handlers/serving.rs` | `serve_model` dispatches directly on two providers. `unserve_model` treats any non-llama.cpp served model as Ollama. The file mixes RPC parsing, validation flow, alias derivation, provider HTTP clients, router restart policy, version-manager lookup, sidecar load/unload calls, GPU-runtime checks, and served-state recording in one ~1.3k-line handler. | Create a provider serving adapter boundary before ONNX. The RPC handler should parse/validate and call a provider adapter for load, unload, provider-side model id, alias defaulting, runtime support checks, and idempotency. Llama.cpp/Ollama behavior should move through the same adapter path so ONNX is not added as a third match branch. |
| Gateway proxy | `rust/crates/pumas-rpc/src/handlers/mod.rs`, `rust/crates/pumas-rpc/src/server.rs` | `/v1/chat/completions`, `/v1/completions`, and `/v1/embeddings` share one blind proxy path. Gateway creates a new `reqwest::Client` per request and rewrites `model` through a two-provider helper. There is no endpoint capability check before proxying. | Add provider endpoint capability checks and a shared gateway client with timeouts/body limits. Unsupported ONNX endpoints must return Pumas-shaped OpenAI-compatible errors without contacting the ONNX sidecar. Provider-side model id mapping belongs to provider behavior, not a gateway match. |
| Runtime lifecycle and launcher | `rust/crates/pumas-core/src/api/state_runtime_profiles.rs`, `rust/crates/pumas-core/src/process/launcher.rs` | Runtime profile launch currently builds binary launch configs for Ollama/llama.cpp and applies llama.cpp-only preparation in lifecycle code. The launcher has separate binary constructors plus a Torch Python constructor that is not integrated into runtime profiles. ONNX is a Python sidecar and should not be forced into the current binary-only runtime-profile path. | Extract a launch strategy abstraction that can produce binary or Python sidecar launch configs through the same lifecycle owner. ONNX managed profiles must use this path with PID/log/health ownership, stale process cleanup, and no provider-specific preparation in generic lifecycle code. |
...
| Torch sidecar reference pattern | `torch-server/*`, Torch process/client handlers | Torch provides a good FastAPI sidecar boundary, validation module, app factory, and model manager. It does not fully solve ONNX needs: OpenAI generation has no explicit inference semaphore, cancellation/shutdown ownership is limited, and Torch integration is app/process-specific rather than runtime-profile-provider-specific. | Use Torch only as a starting pattern. ONNX sidecar must add explicit inference concurrency, shutdown ordering, cancellation/stale-load handling, package-local dependency checks, and runtime-profile integration through the provider model. Do not copy Torch's app-specific integration path as the ONNX provider architecture. |
...
- Separate generic ONNX embedding serving from custom ONNX app bindings
  ...
  artifact is usable by the embedding sidecar.
- Treat app identity, runtime-provider identity, and version-manager identity as
  ...
  use a managed sidecar without duplicating standalone process UI state.
...
- Runtime profile snapshots and route resolution are read-heavy. The current
  ...
  contention.
- ONNX inference must own bounded concurrency and request limits in the sidecar.
  Pumas gateway limits are not a substitute for sidecar queue/thread limits.
