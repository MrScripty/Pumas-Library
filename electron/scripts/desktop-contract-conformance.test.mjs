...
test('runtime-removal decoding preserves exact confirmation booleans', () => {
  ...
  for (const value of [null, true, false, {}, { success: null }, { success: 1 },
    { success: true, error: 'invented' }, { success: false, result: false }]) {
    assert.equal(contract.decodeRemoveVersionOutcome(value).status, 'invalid', JSON.stringify(value));
  }
});
...
test('settings mutation decoding agrees with actual Rust request admission', () => {
  ...
  assert.ok(Array.isArray(probes) && probes.length > 0);
  for (const { params, accepted } of probes) {
    const result = contract.decodeUpdateInferenceSettingsParams(params);
    assert.equal(result.status, accepted ? 'valid' : 'invalid', JSON.stringify(params));
    if (accepted) assert.deepEqual(JSON.parse(JSON.stringify(result.value)), params);
  }
});
