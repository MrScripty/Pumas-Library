...
aliases, runtime profile lifecycle, and gateway routing. ONNX Runtime owns ONNX
...
- Add an ONNX Runtime sidecar for embedding models.
- Add OpenAI-compatible `/v1/models` and `/v1/embeddings` endpoints to the
  ONNX sidecar.
- Add sidecar control endpoints for load, unload, status, devices, and health.
...
- Extend Pumas serving so `serve_model` loads ONNX models through the ONNX
  sidecar and records `ServedModelStatus`.
- Reuse the existing Pumas `/v1/embeddings` gateway by routing served ONNX
  models to their sidecar endpoint.
...
- Add focused Rust, Python, TypeScript, and gateway integration tests.
...
- Pumas `/v1` remains the external facade. External apps should not depend on
  raw sidecar endpoints for normal usage.
...
- Boundary validation must happen before file, network, process, or model-load
  operations.
- ONNX dependencies stay sidecar-local unless another owner demonstrably
  executes them.
...
- Pumas can run a Python ONNX sidecar alongside existing managed runtimes.
- Existing Torch sidecar patterns are useful reference material but not the
  runtime-profile provider architecture.
...
model-library metadata, process launcher, `pumas-rpc` serving/gateway handlers,
frontend app shell/runtime profile/model route UI, plugin metadata, Electron
bridge types, and the existing Torch sidecar reference.

External dependencies include ONNX Runtime, tokenizer/numeric Python packages,
loopback HTTP, OpenAI-compatible embedding payloads, operating system process
management, and platform filesystem behavior.
...
ONNX sidecar control payloads, ONNX embedding payloads, plugin/app metadata,
frontend bridge types, and model-library executable format projections.
...
The runtime profile config moves to a provider-scoped route schema. ONNX sidecar
runtime directories may contain PID files, logs, and sidecar-local config.
Model-library records must classify `.onnx` as a first-class executable format.
...
- The Torch sidecar is a reference pattern only; ONNX still needs explicit
  concurrency, cancellation, shutdown, and runtime-profile provider ownership.
...
| ---- | ------ | ---------- |
| ONNX model output/pooling semantics vary across exports. | High | Add sidecar introspection, explicit postprocess configuration, and shape/numerical tests. |
...
| Sidecar path or request validation is incomplete. | High | Validate paths, aliases, bind hosts, ports, batch sizes, token counts, dimensions, and model ids at boundaries before side effects. |
...
- The Pumas Library sidebar shows an ONNX Runtime app icon with status derived
  from ONNX runtime profile or sidecar state.
...
- The ONNX sidecar validates model paths, model names, request sizes, and bind
...
- Focused Python, Rust, TypeScript, and release/build checks pass.
...
| 1 | ONNX Sidecar Skeleton | Create the package-local ONNX sidecar API, validation boundary, model manager shell, health/status/control endpoints, and OpenAI-compatible response scaffolding. |
| 2 | ONNX Embedding Execution | Add ONNX Runtime/tokenizer/numeric execution with bounded inference, explicit postprocess semantics, request limits, and shape/numerical tests. |
...
| 4 | Serving Validation And Load/Unload | Serve ONNX artifacts through provider adapters, sidecar load/unload, provider-scoped route resolution, and backend-owned served status. |
...
| 7 | Documentation And External App Contract | Document sidecar, provider architecture, runtime profile contracts, gateway examples, and Emily usage. |
...
- `nomic-embed-text-v1.5` ONNX artifacts require custom tokenizer or
  post-processing not represented by the planned sidecar configuration.
...
- ONNX sidecar dependency, native-library, or packaging behavior cannot be
  validated package-locally.
...
- Keep Pumas as the only external endpoint that Emily and other apps configure.
  This preserves backend-owned served state and keeps raw sidecar URLs as
...
- Keep ONNX dependencies sidecar-local and make CPU/GPU package strategy an
