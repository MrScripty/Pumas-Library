...
impl RpcCommand {
    pub(crate) fn method(&self) -> &str {
        match self {
            ...
            #[cfg(feature = "inference-plugins")]
            Self::InstallVersionDependencies { .. } => "install_version_dependencies",
...
pub(crate) enum RpcOutcome {
    ...
    #[cfg(feature = "inference-plugins")]
    InstallVersionDependencies(InstallVersionDependenciesOutcome),
...
impl RpcOutcome {
    ...
    pub(crate) fn into_value(self) -> Result<Value, PublicError> {
        let result = match self {
            ...
            #[cfg(feature = "inference-plugins")]
            Self::InstallVersionDependencies(value) => serde_json::to_value(value),
...
#[derive(Serialize)]
#[cfg_attr(feature = "export-contract", derive(schemars::JsonSchema))]
pub(crate) struct RemoveVersionOutcome {
    success: bool,
}

#[cfg(any(feature = "inference-plugins", feature = "export-contract", test))]
impl RemoveVersionOutcome {
    /// Preserve the manager's removal result without strengthening its guarantees.
    pub(crate) const fn new(success: bool) -> Self {
        Self { success }
    }
}
...
#[cfg(test)]
mod remove_version_tests {
    use super::*;

    #[test]
    fn remove_version_preserves_literal_boolean_outcomes_and_wrapper() {
        for (success, expected) in [
            (true, serde_json::json!({"success": true})),
            (false, serde_json::json!({"success": false})),
        ] {
            let outcome = RemoveVersionOutcome::new(success);
            assert_eq!(serde_json::to_value(&outcome).unwrap(), expected);
            assert_eq!(
                crate::wrapper::wrap_response("remove_version", success.into()),
                expected
            );
            #[cfg(feature = "inference-plugins")]
            {
...
#[derive(Serialize)]
#[cfg_attr(feature = "export-contract", derive(schemars::JsonSchema))]
pub(crate) struct InstalledVersionsOutcome {
    success: bool,
    versions: Vec<String>,
}

#[cfg(any(feature = "inference-plugins", feature = "export-contract", test))]
impl InstalledVersionsOutcome {
    pub(crate) fn new(versions: Vec<String>) -> Self {
        Self {
            success: true,
            versions,
        }
...
#[cfg(test)]
mod installed_versions_tests {
    use super::*;
    #[test]
    fn installed_versions_preserves_exact_list_and_matches_previous_wrapper() {
        for versions in [
            vec![" vλ.1 ".into(), "v2".into(), "v2".into(), String::new()],
            vec![],
        ] {
            let previous = crate::wrapper::wrap_response(
                "get_installed_versions",
                serde_json::to_value(&versions).unwrap(),
            );
            let outcome = InstalledVersionsOutcome::new(versions);
            assert_eq!(serde_json::to_value(&outcome).unwrap(), previous);
            #[cfg(feature = "inference-plugins")]
            {
                let rpc = RpcOutcome::InstalledVersions(outcome);
                assert!(!rpc.uses_response_wrapper());
                assert_eq!(rpc.into_value().unwrap(), previous);
            }
        }
...
#[serde(deny_unknown_fields)]
#[cfg_attr(feature = "export-contract", derive(schemars::JsonSchema))]
pub(crate) struct InstallVersionDependenciesOutcome {
    success: bool,
}

#[cfg(any(feature = "inference-plugins", feature = "export-contract", test))]
impl InstallVersionDependenciesOutcome {
    pub(crate) fn new(success: bool) -> Self {
        Self { success }
    }
}
...
mod runtime_stop_tests {
    ...
    #[test]
    fn runtime_stop_preserves_boolean_handler_and_wrapper_wire() {
        for (raw, expected) in [
            (true, json!({"success":true})),
            (false, json!({"success":false})),
        ] {
            // The old handler wrapped the core bool before the passthrough wrapper.
            let handler_wire = json!({"success":raw});
            assert_eq!(handler_wire, expected);
            ...
            let outcome = RuntimeStopOutcome::new(raw);
            assert_eq!(serde_json::to_value(&outcome).unwrap(), expected);
            #[cfg(feature = "inference-plugins")]
            assert_eq!(
                RpcOutcome::RuntimeStop(outcome).into_value().unwrap(),
                expected
            );
        }
    }
...
#[cfg(test)]
mod install_version_dependencies_tests {
    use super::*;
    use serde_json::json;

    #[test]
    fn install_version_dependencies_admission_preserves_exact_strings() {
        for (params, accepted) in install_version_requests() {
        ...
        assert!(parse_params::<InstallVersionDependenciesParams>(None).is_err());
        assert!(parse_params::<InstallVersionDependenciesParams>(Some(
            &json!({"tag":"v1","app_id":"torch","top_n":2})
        ))
        .is_err());
    }
    ...
    #[test]
    fn install_version_dependencies_preserves_raw_boolean_and_existing_wrapper() {
        for (result, expected) in [
            (true, json!({"success":true})),
            (false, json!({"success":false})),
        ] {
            let raw = serde_json::to_value(result).unwrap();
            assert_eq!(raw, json!(result));
            assert_eq!(
                crate::wrapper::wrap_response("install_version_dependencies", raw),
                expected
            );
            let outcome = InstallVersionDependenciesOutcome::new(result);
            assert_eq!(serde_json::to_value(&outcome).unwrap(), expected);
            #[cfg(feature = "inference-plugins")]
            assert_eq!(
                RpcOutcome::InstallVersionDependencies(outcome)
                    .into_value()
                    .unwrap(),
                expected
            );
...
fn parse_command(method: &str, params: Option<&Value>) -> Result<RpcCommand, PublicError> {
    ...
    match method {
        ...
        #[cfg(feature = "inference-plugins")]
        "stop_torch" => empty().map(|()| RpcCommand::StopTorch),
        #[cfg(not(feature = "inference-plugins"))]
        "stop_ollama" | "stop_torch" => Err(PublicError::method_not_found()),
        #[cfg(feature = "inference-plugins")]
        ...
        #[cfg(feature = "inference-plugins")]
        "install_version_dependencies" => parse_params::<InstallVersionDependenciesParams>(params)
            .map(|params| RpcCommand::InstallVersionDependencies {
                app_id: params.app_id,
                tag: params.tag,
            }),
        #[cfg(not(feature = "inference-plugins"))]
        "install_version_dependencies" => Err(PublicError::method_not_found()),
        #[cfg(feature = "inference-plugins")]
        "install_version" => {
