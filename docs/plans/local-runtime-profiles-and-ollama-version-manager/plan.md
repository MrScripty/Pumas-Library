...
- 2026-05-09: The llama.cpp app page and sidebar did not consume backend serving status outside the serve dialog. A loaded model in the runtime-profile library panel still showed only a Play action, and the llama.cpp app icon stayed offline because its managed-app state was hardcoded to not running. Resolved by adding a renderer serving-status snapshot/update-feed hook, passing served-model state into local model rows, and deriving the llama.cpp icon running state from llama.cpp runtime profiles plus loaded served models.
- 2026-05-09: Live unload testing found router-mode `unserve_model` only removed Pumas' served-model record and did not call llama.cpp's router unload API, so the GUI could report the model as not served while hardware memory stayed allocated. Resolved by posting to the selected router endpoint's `/models/unload` API before clearing the backend served-model snapshot, and by preserving the served state with a non-critical error if the provider unload fails.
- 2026-05-09: Live GPU-profile testing found llama.cpp profiles with `device.mode = gpu` but blank GPU layers still launched without a GPU offload argument, so llama.cpp defaulted to CPU. A running router launched with older CPU arguments also kept spawning CPU child servers after the saved profile changed to GPU. Resolved by treating blank GPU/specific-device layers as full offload (`--n-gpu-layers -1`) and by restarting idle managed routers before load when explicit device settings must be applied.
- 2026-05-09: Live GPU-profile tracing found the installed `b9082` llama.cpp runtime itself was the CPU-only Ubuntu archive. `llama-server --list-devices` reported no devices and the runtime directory contained CPU/RPC backends but no Vulkan/CUDA/ROCm backend library, so a GPU profile could only load tensors into system RAM even when Pumas passed GPU offload flags. Resolved by preferring GPU-capable llama.cpp release archives on GPU hosts, recording the selected archive metadata, showing release sizes for that selected archive, and returning a non-critical `device_unavailable` serving error when a GPU request targets a CPU-only llama.cpp runtime.
- 2026-05-09: The previous GPU-archive preference was still too implicit because llama.cpp publishes CPU, Vulkan, ROCm, CUDA, HIP, and SYCL binaries as separate assets under the same release tag. A single install row made it unclear which runtime would be downloaded and prevented CPU and GPU-capable runtimes from being installed side by side. Resolved by expanding llama.cpp releases into separate installable runtime variant rows such as `b9082+cpu`, `b9082+vulkan`, and `b9082+rocm`, each with its own archive size, cache file, install directory, and active-version entry.
- 2026-05-09: Installing a Linux SYCL runtime exposed a duplicate install-page entry because upstream publishes both `sycl-fp32` and `sycl-fp16` archives, but the first variant expansion collapsed both into the same `+sycl` tag. Resolved by assigning precision-specific tags (`+sycl-fp32`, `+sycl-fp16`) and migrating legacy installed `+sycl` metadata/directories to the precision-specific tag based on the recorded download URL.
- 2026-05-09: Live GPU-profile tracing against `Emily Lamacpp` found the installed `b9090+vulkan` archive was present, but the app launched the copied `bin/llama-server` outside the extracted archive directory. That copied binary could not resolve `libllama-common.so.0`, while the archive-local `llama-b9090/llama-server --list-devices` loaded the Vulkan backend. Resolved by making process launch prefer archive-local `llama-server` binaries whose directory contains llama.cpp runtime libraries, by adding the binary directory to the child library search path, and by changing future installs to create a small launch wrapper instead of copying the binary away from its shared libraries.
- 2026-05-09: The same trace showed a stale router child spawned with `--n-gpu-layers 0`. Router-mode serving had not passed the model serve request's device/context overrides into the profile launch path, so a GPU serve request could still reuse or relaunch router CPU arguments. Resolved by applying the existing llama.cpp launch override path to router profiles as well as dedicated profiles, replacing stale CPU layer args with the selected GPU/default full-offload args before launch.
- 2026-05-09: Follow-up live host process inspection found an orphaned May 8 `b9082` router still bound to Emily's derived endpoint with no `runtime.pid`, plus a child model server launched with `--n-gpu-layers 0`. The managed launch readiness check accepted the old endpoint as proof that the new launch succeeded even if the newly spawned process exited because the port was already owned. Resolved by cleaning profile-owned orphan processes that reference the profile runtime directory before launch, and by failing a launch if the spawned child exits before it becomes the managed runtime.
...
- 2026-05-09 continuation: wired backend serving state into the llama.cpp app page and sidebar. Loaded models now show a `Loaded` badge and active serve action in the bottom library panel, and the llama.cpp app icon enters the running spinner state when a llama.cpp profile is running or a llama.cpp model is loaded.
- 2026-05-09 continuation: fixed llama.cpp router unload semantics. Pumas now asks the router to unload the selected model before removing it from backend serving status, preventing the GUI from hiding a still-resident model after an unload click.
- 2026-05-09 continuation: fixed llama.cpp GPU profile placement. GPU and specific-device profiles now default blank GPU layers to `-1` for full offload, the profile editor persists that default visibly, dedicated serve overrides use the same default, and idle managed routers restart before serving so stale CPU launch arguments are not reused.
- 2026-05-09 continuation: fixed llama.cpp GPU runtime selection. The installer and release metadata now prefer GPU-capable llama.cpp archives on GPU hosts, and serving now rejects GPU-profile loads against CPU-only llama.cpp runtimes with a non-critical domain error instead of silently loading into RAM.
- 2026-05-09 continuation: split llama.cpp release tags into explicit runtime variants so users can install and activate CPU, Vulkan, ROCm, CUDA, HIP, or SYCL builds independently when those assets exist for the current platform.
- 2026-05-09 continuation: split Linux SYCL runtime variants by precision and added metadata migration for existing `+sycl` installs so an installed FP16 archive is recognized as `+sycl-fp16` instead of appearing as a duplicate stale runtime.
- 2026-05-09 continuation: fixed llama.cpp Vulkan archive launching for `Emily Lamacpp`. Existing installs now launch the archive-local `llama-server` with its runtime library directory in the library search path, future installs write a wrapper instead of copying the binary away from its `.so` files, and router profile launches apply the selected model/device overrides so GPU requests do not preserve stale `--n-gpu-layers 0` arguments.
- 2026-05-09 continuation: fixed stale managed llama.cpp router reuse. Managed profile startup now removes orphaned processes that still reference the profile runtime directory when no profile PID file exists, and process launch no longer treats a ready port as success if the child process already exited. This prevents an old CPU router from masquerading as the current Vulkan Emily profile.
...
- 2026-05-09 serving UI state validation: `npm run -w frontend test:run -- LocalModelsList LocalModelInstalledActions useManagedApps AppShellState`, `npm run -w frontend check:types`, and `npm run -w frontend build` passed.
- 2026-05-09 router unload validation: Direct live router testing confirmed `POST http://127.0.0.1:20617/models/unload` unloaded `embedding/qwen3/qwen3-embedding-06b-gguf`; `/v1/models` then reported the model as unloaded, and the router child `llama-server` listener was gone. Code and release validation passed with `cargo fmt --manifest-path rust/crates/pumas-rpc/Cargo.toml`, `cargo test --manifest-path rust/crates/pumas-rpc/Cargo.toml llama_cpp_router_model_unload_url_normalizes_trailing_slash`, `cargo check --manifest-path rust/crates/pumas-rpc/Cargo.toml`, and `bash launcher.sh --build-release`.
- 2026-05-09 llama.cpp GPU profile validation: Existing runtime logs confirmed the stale router spawned a child server with `--n-gpu-layers 0` and offloaded `0/49` layers despite the saved profile now being `device.mode = gpu`. The fix was validated with `cargo fmt --manifest-path rust/crates/pumas-core/Cargo.toml`, `cargo test --manifest-path rust/crates/pumas-core/Cargo.toml runtime_profile_service_defaults_llama_cpp_gpu_to_full_offload`, `cargo test --manifest-path rust/crates/pumas-core/Cargo.toml llama_cpp_dedicated_gpu_override_defaults_to_full_offload`, `cargo test --manifest-path rust/crates/pumas-rpc/Cargo.toml llama_cpp_router_profile_restart_detects_explicit_device_settings`, `npm run -w frontend test:run -- RuntimeProfileSettingsSection`, `npm run -w frontend check:types`, `cargo check --manifest-path rust/crates/pumas-rpc/Cargo.toml`, `npm run -w frontend build`, and `bash launcher.sh --build-release`.
- 2026-05-09 llama.cpp GPU runtime validation: Live runtime tracing confirmed the installed `b9082` binary loaded only `libggml-rpc.so` and `libggml-cpu-alderlake.so`, `--list-devices` reported no devices, and the runtime directory had no Vulkan/CUDA/ROCm backend libraries. The fix was validated with `cargo fmt --manifest-path rust/crates/pumas-app-manager/Cargo.toml`, `cargo fmt --manifest-path rust/crates/pumas-rpc/Cargo.toml`, `cargo fmt --manifest-path rust/crates/pumas-core/Cargo.toml`, `cargo test --manifest-path rust/crates/pumas-app-manager/Cargo.toml llama_cpp_asset_selection`, `cargo test --manifest-path rust/crates/pumas-core/Cargo.toml llama_cpp_asset_selection`, `cargo test --manifest-path rust/crates/pumas-rpc/Cargo.toml llama_cpp_runtime`, `cargo check --manifest-path rust/crates/pumas-rpc/Cargo.toml`, and `bash launcher.sh --build-release`.
- 2026-05-09 llama.cpp runtime variant validation: `cargo fmt --manifest-path rust/crates/pumas-core/Cargo.toml`, `cargo fmt --manifest-path rust/crates/pumas-app-manager/Cargo.toml`, `cargo test --manifest-path rust/crates/pumas-core/Cargo.toml llama_cpp_release_expansion`, `cargo test --manifest-path rust/crates/pumas-app-manager/Cargo.toml llama_cpp_asset_selection`, `npm run -w frontend test:run -- VersionListItem`, `cargo check --manifest-path rust/crates/pumas-rpc/Cargo.toml`, `npm run -w frontend check:types`, and `bash launcher.sh --build-release` passed.
- 2026-05-09 llama.cpp SYCL variant validation: `cargo fmt --manifest-path rust/crates/pumas-core/Cargo.toml`, `cargo fmt --manifest-path rust/crates/pumas-app-manager/Cargo.toml`, `cargo test --manifest-path rust/crates/pumas-core/Cargo.toml llama_cpp_release_expansion`, `cargo test --manifest-path rust/crates/pumas-app-manager/Cargo.toml legacy_sycl`, `cargo check --manifest-path rust/crates/pumas-rpc/Cargo.toml`, and `bash launcher.sh --build-release` passed.
- 2026-05-09 llama.cpp Vulkan launch validation: `ldd llama-cpp-versions/b9090+vulkan/bin/llama-server` confirmed the copied launch binary could not resolve llama.cpp shared libraries, while `llama-cpp-versions/b9090+vulkan/llama-b9090/llama-server --list-devices` loaded `libggml-vulkan.so`. The fix was validated with `cargo fmt --manifest-path rust/Cargo.toml --all`, `cargo test --manifest-path rust/Cargo.toml -p pumas-library test_llama_cpp_router_prefers_archive_binary_with_runtime_libraries`, `cargo test --manifest-path rust/Cargo.toml -p pumas-library llama_cpp_router_override_replaces_stale_cpu_layers`, `cargo test --manifest-path rust/Cargo.toml -p pumas-app-manager llama_cpp_launch_binary_uses_wrapper_with_runtime_library_path`, `bash launcher.sh --build-release`, and `bash launcher.sh --release-smoke`.
- 2026-05-09 stale router validation: Host `ps` output confirmed the active router was still `/llama-cpp-versions/b9082/llama-b9082/llama-server --port 20617 ... --n-gpu-layers 0` with no Emily `runtime.pid`. The fix was validated with `cargo fmt --manifest-path rust/Cargo.toml --all`, `cargo test --manifest-path rust/Cargo.toml -p pumas-library test_binary_launch_fails_when_spawned_process_exits_immediately`, `cargo test --manifest-path rust/Cargo.toml -p pumas-library llama_cpp_router_override_replaces_stale_cpu_layers`, `cargo test --manifest-path rust/Cargo.toml -p pumas-library test_llama_cpp_router_prefers_archive_binary_with_runtime_libraries`, `bash launcher.sh --build-release`, and `bash launcher.sh --release-smoke`.
