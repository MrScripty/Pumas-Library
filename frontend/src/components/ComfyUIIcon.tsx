...
const RunningIcon: React.FC<{ ramUsage?: number; gpuUsage?: number; isSelected?: boolean }> = ({
  ramUsage = 60,
  gpuUsage = 40,
  isSelected = false
}) => {
...
const OfflineIcon: React.FC<{ isSelected?: boolean }> = ({ isSelected = false }) => {
  return (
    <IconWrapper isSelected={isSelected}>
...
const ErrorIcon: React.FC<{ isSelected?: boolean }> = ({ isSelected = false }) => {
  return (
    <IconWrapper isSelected={isSelected}>
      <div className="w-full aspect-square bg-[hsl(var(--launcher-bg-tertiary)/0.3)] hover:bg-[hsl(var(--launcher-bg-tertiary)/0.5)] transition-colors flex items-center justify-center border-[hsl(var(--launcher-border))] hover:border-[hsl(var(--launcher-border)/0.8)] cursor-pointer relative border-2 rounded-full opacity-80">
        <div className="absolute inset-0 rounded-full overflow-hidden">
...
const UninstalledIcon: React.FC<{ isSelected?: boolean }> = ({ isSelected = false }) => {
  return (
    <IconWrapper isSelected={isSelected}>
  ...
  return (
    <div className="relative w-full overflow-visible hover:z-50" title={title}>
      <button
        ...
        aria-label={title ?? 'ComfyUI'}
      >
        {state === 'running' ? (
          <RunningIcon ramUsage={ramUsage} gpuUsage={gpuUsage} isSelected={isSelected} />
        ) : state === 'offline' ? (
          <OfflineIcon isSelected={isSelected} />
        ) : state === 'uninstalled' ? (
          <UninstalledIcon isSelected={isSelected} />
        ) : (
          <ErrorIcon isSelected={isSelected} />
        )}
      </button>
