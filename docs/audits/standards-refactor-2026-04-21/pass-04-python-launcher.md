...
- Completed: `launcher.sh --run-release` now launches the current release build outputs directly with the repository-local Electron runtime when `frontend/dist`, `electron/dist`, and `rust/target/release/pumas-rpc` are present. This avoids both stale packaged-app drift and desktop-launch failures caused by GUI sessions missing `node`, `corepack`, or `cargo` on `PATH`.
