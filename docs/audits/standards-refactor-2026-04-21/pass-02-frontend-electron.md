...
- Completed: keep retained partial-download progress visualization in the local model row state, render indexed partial downloads with the same ring surface in a grey treatment even after numeric progress has fallen out of the library projection, and keep paused/error download rows on the download-actions path showing the grey ring instead of dropping the indicator entirely, with focused row-state and action tests.
