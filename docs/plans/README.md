...
| `pumas-artifact-load-target-resolution/` | Accepted Pantograph-facing proposal and implementation plan for resolving selected Pumas artifacts into typed local load targets without leaking storage ownership. |
| `backend-owned-status-resource-telemetry/plan.md` | Plan for replacing idle status/resource polling with backend-owned cached telemetry snapshots, plus active remediation for remaining download/runtime-profile polling and backend runtime thread budget issues. |
