...
const RunningIcon: React.FC<{ appId: string; ramUsage?: number; gpuUsage?: number }> = ({
  appId,
  ramUsage = 60,
  gpuUsage = 40
}) => {
...
const OfflineIcon: React.FC<{ appId: string }> = ({ appId }) => {
  return (
...
const ErrorIcon: React.FC<{ appId: string }> = ({ appId }) => {
  return (
    <div className="w-full aspect-square bg-[hsl(var(--launcher-bg-tertiary)/0.3)] hover:bg-[hsl(var(--launcher-bg-tertiary)/0.5)] transition-colors flex items-center justify-center border-[hsl(var(--launcher-border))] hover:border-[hsl(var(--launcher-border)/0.8)] cursor-pointer relative border-2 rounded-full opacity-80">
      <div className="absolute inset-0 rounded-full overflow-hidden">
        <img
...
const UninstalledIcon: React.FC<{ appId: string }> = ({ appId }) => {
  return (
  ...
  return (
    ...
    >
      <div className="relative overflow-visible">
        {state === 'running' ? (
          <RunningIcon appId={appId} ramUsage={ramUsage} gpuUsage={gpuUsage} />
        ) : state === 'offline' ? (
          <OfflineIcon appId={appId} />
        ) : state === 'uninstalled' ? (
          <UninstalledIcon appId={appId} />
        ) : (
          <ErrorIcon appId={appId} />
        )}
