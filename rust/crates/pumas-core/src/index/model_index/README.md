...
| `model_selector_snapshot.rs` | `ModelIndex` projection for fast model-library selector snapshots from indexed SQLite/cache state. |
...
- Selector snapshots must be cheap indexed projections. They may use SQLite model rows and cache rows, but must not scan model directories, regenerate package facts, resolve dependencies, or call public per-model APIs.
...
- Selector rows must keep `indexed_path` as display/debug data only and must expose an executable `entry_path` only when both entry and artifact state are ready.
...
- `model_selector_snapshot` rows are list projections over existing indexed facts. They may report missing or invalid package summaries without forcing regeneration.
